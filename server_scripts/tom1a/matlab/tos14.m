...
function Outputs(block)
	...
	% output_path = block.DialogPrm(7).Data;
        ...
        newValues = values(beginPos:endPos);
        % values = strcat(values,outputPath);
        fid = fopen(outputPath,'a+');
        % fid = fopen(output_path,'a+');
        %fprintf(fid,'%s\n',newValues);
        fprintf(fid,'%s\n',strcat(num2str(block.CurrentTime),',',newValues,',',num2str(block.InputPort(1).Data),',',num2str(block.InputPort(3).Data),',',num2str(block.InputPort(2).Data),',',num2str(block.InputPort(4).Data)));
        fclose(fid);
    catch err
    ...
    if (crc == intchecksum)
      ...
      val = sscanf(body, '%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g');
      
      block.OutputPort(1).Data = val(1);
      block.OutputPort(2).Data = val(2);
      block.OutputPort(3).Data = val(3);
      block.OutputPort(4).Data = val(6);
      block.OutputPort(5).Data = val(7);
      block.OutputPort(6).Data = val(14);
      block.OutputPort(7).Data = val(15);
    end
%end Outputs
